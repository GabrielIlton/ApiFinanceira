...
class AccountService {
    ...
    async depositAccount ({ body, token }) {
        ...
        const createDeposit = await StatementRepository.depositStatement({ deposit: body.deposit, token });
    ...
    async withDrawAccount ({ body, token }) {
        ...
        if(body.withDraw <= accountBalance.balance){
            await StatementRepository.withDrawStatement({ withDraw: body.withDraw, token });
    ...
    async P2P ({ body, token }) {
        ...
        const P2Pcashout = await TransactionRepository.P2Pcashout({ accountSend, amount: body.amount });
        ...
        await AccountRepository.findOneAndUpdateBalance({ id: accountIdSend.id, total: totalSend });

        const P2Pcashin = await TransactionRepository.P2Pcashin({ accountReciever, amount: body.amount });
        ...
        await AccountRepository.findOneAndUpdateBalance({ id: accountIdReciever.id, total: totalReciever });

        const StatementCashoutP2P = await StatementRepository.P2Pcashout({ amount: body.amount, accountSend })
        const StatementCashinP2P = await StatementRepository.P2Pcashin({ amount: body.amount, accountReciever })

        return res.status(201).json({ accountSend: account.name, cashoutSend: req.body.amount, balanceSend: account.balance, Reciever: req.body.accountReciever.name, cashinReciever: req.body.amount});
